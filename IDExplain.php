...
<?php 
    function  curl_302($url, $vars = null) {
        $ch = curl_init();
            ...
            if ( $result['code'] == 200 && $result[$key] ){
                ...
                foreach ( $data as $keys => $data ){
                    ...
                    $list[$data['id']] = array(
                            ...
                            //'location' => str_replace('http://m', 'http://p', $data['mp3Url']), //已失效
			    //'location' => str_replace('http://', 'https://', curl_302('https://music.163.com/song/media/outer/url?id='.$data['id'].'.mp3')),
			                'location' => $data['id'],
